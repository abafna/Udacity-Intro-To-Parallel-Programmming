...
{
	// Get thread position within the image.
	const int2 thread_2D_pos = make_int2( blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);

	// Discard threads not pertaining to image bounds.
	if (thread_2D_pos.x >= numCols || thread_2D_pos.y >= numRows) return;

	// Assert width of filter is odd.
	assert(filterWidth % 2 == 1);

	float result = 0.f;
	//For every value in the filter around the pixel.
	for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; ++filter_r)
	{
		for (int filter_c = -filterWidth/2; filter_c <= filterWidth/2; ++filter_c)
		{
			//Find the global image position for this filter position
			//clamp to boundary of the image (from reference_calc.cpp)
                      ...
                      unsigned char* const blueChannel)
{
	...
	uchar4 pixelRGBA  = inputImageRGBA[thread_1D_pos];

	redChannel[thread_1D_pos] = pixelRGBA.x;
	greenChannel[thread_1D_pos] = pixelRGBA.y;
	blueChannel[thread_1D_pos] = pixelRGBA.z;
}
                        ...
                        const int filterWidth)
{
  ...
  //Launch a kernel for recombining different color channels.
...
//
//Your code executed in 2.96768 ms 
